...
def launch(args):
    ...
    # --- wandb init ---
    use_wandb = WANDB_AVAILABLE and not getattr(args, 'no_wandb', False)
    if use_wandb:
        run_name = args.wandb_name if args.wandb_name else f"{args.env_name}-{ckpt_name}"
        wandb.init(
